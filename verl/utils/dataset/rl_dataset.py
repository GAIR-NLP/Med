...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.num_workers = (
            min(self.num_workers, os.cpu_count()) if self.num_workers is not None else None
        )
    ...
    def _read_files_and_tokenize(self):
        ...
        for parquet_file in self.data_files:
            ...
            dataframe = datasets.load_dataset(
                "parquet", data_files=parquet_file, num_proc=8, split="train"
            )
            len_ += len(dataframe)
                    ...
                    try:
                        ...
                        return len(
                            tokenizer.apply_chat_template(
                                doc[prompt_key], add_generation_prompt=True, **apply_kwargs
                            )
                        )
                    except Exception:
                        print("Error processing one of the samples, skipping...")
    ...
    def __getitem__(self, item):
        ...
        verifier = row_dict["reward_model"]["verifier"]
        if self.config.enforce_single_turn:
            if row_dict.get("extra_info", {}).get("need_tools_kwargs", False):
                row_dict["extra_info"]["need_tools_kwargs"] = False
                row_dict["agent_name"] = "single_turn_agent"
        need_tools_kwargs: bool = row_dict.get("extra_info", {}).get("need_tools_kwargs", False)
        agent_name: bool = row_dict.get("agent_name", "single_turn_agent")

        if agent_name == "tool_agent":
            # A hack to enable new image_crop_and_zoom_in tool
            row_dict["extra_info"]["tools_kwargs"]["image_crop_and_zoom_in_tool"] = row_dict[
                "extra_info"
            ]["tools_kwargs"].pop("crop_and_zoom")
            tool_names = list(row_dict["extra_info"]["tools_kwargs"].keys())
            tool_prompt = get_tool_calling_prompt(tool_names)
            row_dict[self.prompt_key][-1]["content"] += "\n\n" + tool_prompt

        think_prompt = get_thinking_prompt(answer, verifier)
        row_dict[self.prompt_key][-1]["content"] += "\n\n" + think_prompt
        ...
        if self.processor is not None:
            ...
            if row_dict_images:
                ...
                if self.config.get("enable_multimodal_mask", False) and self.is_train:
                    images = [random_square_block_mask(image) for image in images]
            ...
            if row_dict_videos:
                ...
                multi_modal_data["video"] = [
                    (video.numpy(), metadata)
                    for video, metadata in zip(videos, video_metadata, strict=True)
                ]
        ...
        elif self.processor is not None and (
            "Glm46VImageProcessor" in self.processor.image_processor.__class__.__name__
            or "Glm4vImageProcessor" in self.processor.image_processor.__class__.__name__
        ):
        ...
        # Process tools_kwargs to set raw_query from prompt content
        if tools_kwargs:
            for tool_name, tool_config in tools_kwargs.items():
                if isinstance(tool_config, dict) and "create_kwargs" in tool_config:
                    ...
                    if isinstance(create_kwargs, dict) and "raw_query" in create_kwargs:
                        raw_query = ""
                        for content in messages[0]["content"]:
                            if content["type"] == "text":
                                raw_query += content["text"]
                        create_kwargs["raw_query"] = raw_query.strip()
