...
class MultiTurnSFTDataset(Dataset):
    ...
    def __init__(
        self, parquet_files: str | list[str], tokenizer, config=None, max_samples: int = -1
    ):
    ...
    def __getitem__(self, item):
        ...
        while i < len(messages):
            ...
            if override_loss_mask is not None:
                ...
                assert isinstance(
                    override_loss_mask, int
                ), f"loss_mask should be int, got {type(override_loss_mask)}"
                assert override_loss_mask in [
                    0,
                    1,
                ], f"loss_mask should be 0 or 1, got {override_loss_mask}"
        ...
        if self.pad_mode == DatasetPadMode.RIGHT:
            if sequence_length < self.max_length:
                ...
                padded_input_ids = torch.full(
                    (self.max_length - sequence_length,), pad_token_id, dtype=input_ids.dtype
                )
