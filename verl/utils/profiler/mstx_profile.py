...
def get_npu_profiler(
    ...
    if profile_level == "level_none":
        ...
        raise ValueError(
            f"level only supports level0, 1, 2, and level_none, but gets {profile_level}"
        )
    ...
    prof = torch_npu.profiler.profile(
        ...
        on_trace_ready=torch_npu.profiler.tensorboard_trace_handler(
            profile_save_path, analyse_flag=analysis
        ),
...
class NPUProfiler(DistProfiler):
    ...
    def annotate(
        self, message: Optional[str] = None, role: Optional[str] = None, **kwargs_outer
    ) -> Callable:
