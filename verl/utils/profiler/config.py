...
class TorchProfilerToolConfig(BaseConfig):
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(
            self.step_start, int
        ), f"Profiler step_start must be of type int, got {type(self.step_start)}"
...
class TorchMemoryToolConfig(BaseConfig):
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(
            self.stack_depth, int
        ), f"stack_depth must be int, got {type(self.stack_depth)}"
        assert (
...
class NPUToolConfig(NsightToolConfig):
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(
            self.level, str
        ), f"Profiler level must be of type str, got {type(self.level)}"
        assert isinstance(
            self.analysis, bool
        ), f"Profiler analysis must be of type bool, got {type(self.analysis)}"
        for content in self.contents:
            assert (
                content in ["npu", "cpu", "memory", "shapes", "module", "stack"]
...
class ProfilerConfig(BaseConfig):
    ...
    def union(self, other: "ProfilerConfig") -> "ProfilerConfig":
        ...
        ), f"Cannot union ProfilerConfig with different tools: {self.tool} vs {other.tool}"
    ...
    def intersect(self, other: "ProfilerConfig") -> "ProfilerConfig":
        ...
        return ProfilerConfig(
            ...
            if self.global_tool_config
            else other.global_tool_config,
        )
