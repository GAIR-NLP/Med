...
def get_tensordict(
    tensor_dict: dict[str, torch.Tensor | list], non_tensor_dict: dict = None
) -> TensorDict:
    """
        ...
        for key, tensor in batch.items():
            if isinstance(tensor, torch.Tensor) and not tensor.is_nested:
                ...
                data_dict[key] = torch.nested.as_nested_tensor(
                    [tensor[idx] for idx in indices], layout=torch.jagged
                )
            else:
...
def make_iterator(
    tensordict: TensorDict, mini_batch_size, epochs, seed=None, dataloader_kwargs=None
):
    from torch.utils.data import DataLoader

    assert (
        tensordict.batch_size[0] % mini_batch_size == 0
    ), f"{tensordict.batch_size[0]} % {mini_batch_size} != 0"
...
def assert_tensordict_eq(tensordict1: TensorDict, tensordict2: TensorDict):
    ...
    for key in tensordict1.keys():
        ...
        assert type(val) is type(
            val2
        ), f"The type of {key} must be the same. Got {type(val)} vs {type(val2)}"
        ...
        if isinstance(val, torch.Tensor):
            if val.is_nested:
                ...
                assert len(t1) == len(
                    t2
                ), f"Nested tensor should have the same lengths. {len(t1)=} vs {len(t2)=}"
                for c1, c2 in zip(t1, t2, strict=True):
                    assert torch.equal(
                        c1, c2
                    ), f"Nested tensor components have different values. {c1=} vs {c2=}"
            else:
