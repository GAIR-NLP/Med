...
def create_transferqueue_client(
    ...
    if "val" in client_id:
        _VAL_TRANSFER_QUEUE_CLIENT = AsyncTransferQueueClient(
            client_id, controller_infos, storage_infos
        )
    else:
        _TRANSFER_QUEUE_CLIENT = AsyncTransferQueueClient(
            client_id, controller_infos, storage_infos
        )
    ...
    def decorator(func):
        ...
        def inner(*args, **kwargs):
            ...
            if batchmeta is None:
                ...
                args = [
                    _batchmeta_to_dataproto(arg) if isinstance(arg, BatchMeta) else arg
                    for arg in args
                ]
                kwargs = {
                    k: _batchmeta_to_dataproto(v) if isinstance(v, BatchMeta) else v
        ...
        async def async_inner(*args, **kwargs):
            ...
            if batchmeta is None:
                ...
                args = [
                    await _async_batchmeta_to_dataproto(arg) if isinstance(arg, BatchMeta) else arg
                    for arg in args
                ]
                kwargs = {
