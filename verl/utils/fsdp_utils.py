...
if version.parse(torch.__version__) >= version.parse("2.6"):
    ...
    fully_shard, MixedPrecisionPolicy, FSDPModule, CPUOffloadPolicy, fully_shard_module = (
        None,
        None,
        None,
        None,
    ...
    def create_and_sync_state(param_name, state, is_param):
        ...
        if is_param:
            param = torch.nn.Parameter(
                torch.empty_like(state.data, device=device), requires_grad=state.requires_grad
            )
        else:  # buffer
...
def apply_fsdp2(model, fsdp_kwargs, config):
    ...
    if model.__class__.__name__ == "InternVLForConditionalGeneration":
        fsdp_transformer_layer_cls_to_wrap = ["InternVLVisionLayer", "Qwen3DecoderLayer"]

    if (
        model.__class__.__name__ == "Glm46VForConditionalGeneration"
        or model.__class__.__name__ == "Glm4vForConditionalGeneration"
    ):
        fsdp_transformer_layer_cls_to_wrap = ["Glm4vVisionBlock", "Glm4vTextDecoderLayer"]
    ...
    for _, module in enumerate(modules):
        if (
            "Block" in module.__class__.__name__
            or "InternVLVisionLayer" in module.__class__.__name__
        ):
            continue
