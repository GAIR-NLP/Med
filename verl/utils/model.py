...
def get_model_size(model: nn.Module, scale="auto"):
    ...
    Args:
        ...
        scale: The scale for displaying the number of parameters ('auto', 'B', 'M', 'K', '').
               'auto' determines the scale based on the total number of parameters.

    Returns:
        A tuple containing:
            - n_params (float): Scaled total number of parameters.
            - trainable_params (float): Scaled number of trainable parameters.
            - scale (str): The scale unit used ('B', 'M', 'K', '').
    """
    n_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

    if scale == "auto":
        if n_params >= 1e9:  # Use >= for clearer boundaries
            scale = "B"
        elif n_params >= 1e6:
            scale = "M"
        elif n_params >= 1e3:
    ...
    if scale == "B":
        ...
        n_params /= 1e3
        trainable_params /= 1e3
    elif scale == "":
        # Keep parameters as integers if no scaling is applied
        n_params = int(n_params)
        trainable_params = int(trainable_params)
    else:
        # Fix: Use NotImplementedError instead of NotImplemented
        raise NotImplementedErrorError(f"Unknown scale {scale}")

    return n_params, trainable_params, scale


def print_model_size(model: nn.Module, name: str = None):
    """Prints the total and trainable parameter count of a model.

    Args:
        model: The PyTorch model.
        name: Optional name for the model. If None, uses the model's class name.
    """
    n_params, trainable_params, scale = get_model_size(model, scale="auto")
    if name is None:
        name = model.__class__.__name__
    # Fix: Break long line to conform to style guides
    print(
        f"{name} contains {n_params:.2f}{scale} parameters, "
        f"{trainable_params:.2f}{scale} parameters are trainable"
    )
...
def load_megatron_model_weights(
    config, model_config, parallel_model, params_dtype, is_value_model=False
):
    """Load weights for verl customized model."""
    architectures, model, state_dict, is_value_model = _load_hf_model(
        config, model_config, is_value_model
    )
