...
def get_model(
    ...
    if (
        ...
        model = []
        has_vp_stage = (
            inspect.signature(mpu.is_pipeline_first_stage).parameters.get("vp_stage", None)
            is not None
        )
...
def broadcast_from_megatron_pp(tensor: torch.Tensor):
    ...
    if tensor is None:
        tensor = torch.empty(
            size=target_tensor_spec[0], dtype=target_tensor_spec[1], device=get_device_id()
        )
    ...
    for cur_pp_rank, scan_vpp_idx, idx, name in layer_list_meta:
        ...
        if ".mlp.experts.linear_fc" in cur_name and ep_size > 1:
            ...
            for name, param in zip(global_expert_names, infer_params, strict=True):
                ...
                yield from zip(
                    converted_names, [param.detach() for param in converted_params], strict=True
                )
            continue
...
def get_transformer_layer_offset(pipeline_rank, vp_stage, config: TransformerConfig):
    ...
    has_vp_stage = (
        inspect.signature(parallel_state.is_pipeline_first_stage).parameters.get("vp_stage", None)
        is not None
    )
