...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        with get_fsdp_state_ctx(
            ...
            if self.should_load_model:
                ...
                log_with_rank(
                    f"Loaded model from {remote_model_path}", rank=self.rank, logger=logger
                )
            ...
            if self.should_load_optimizer:
                ...
                log_with_rank(
                    f"Loaded optimizer from {remote_optim_path}", rank=self.rank, logger=logger
                )
        ...
        if self.should_load_extra:
            ...
            if "rng" in extra_state_dict:
                ...
                log_with_rank(
                    f"Loaded rng from {remote_extra_state_path}", rank=self.rank, logger=logger
                )
    ...
    def save_checkpoint(
        self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None
    ):
        """
