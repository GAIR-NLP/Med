...
def update_prometheus_config(config: PrometheusConfig, server_addresses: list[str]):
    ...
    try:
        ...
        def reload_prometheus(port):
            ...
            try:
                subprocess.run(
                    ["curl", "-X", "POST", reload_url], capture_output=True, text=True, timeout=10
                )
        ...
        print(
            f"Updated Prometheus configuration at {config.file} with {len(server_addresses)} VLLM servers"
        )
