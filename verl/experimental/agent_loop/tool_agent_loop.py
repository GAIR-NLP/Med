...
class AgentData:
    ...
    def __init__(
        ...
        self.tool_call_counts = 0  # Add tool_call_counts
...
class ToolAgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        ...
        if cls.interaction_config_file:
            cls.interaction_map: dict[str, BaseInteraction] = cls._initialize_interactions(
                cls.interaction_config_file
            )
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        # Initialize conversation tracking in agent_data
        agent_data.full_messages = copy.deepcopy(list(kwargs["raw_prompt"]))
        agent_data.tool_metrics = {}
        ...
        multi_modal_data = (
            {"image": agent_data.image_data} if agent_data.image_data is not None else {}
        )
        output = AgentLoopOutput(
            ...
            messages={"messages": agent_data.full_messages},
            tool_call_counts=agent_data.tool_call_counts,
            extra_fields={},
        )
    ...
    async def _handle_pending_state(
        ...
        if self.processor is not None:
            ...
            model_inputs = self.processor(
                text=[raw_prompt], images=agent_data.image_data, return_tensors="pt"
            )
    ...
    async def _handle_generating_state(
        ...
        # Add assistant response to full_messages
        response_text = await self.loop.run_in_executor(
            None, lambda: self.tokenizer.decode(agent_data.response_ids, skip_special_tokens=True)
        )
        if response_text.strip():
            agent_data.full_messages.append(
                {"role": "assistant", "content": [{"type": "text", "text": response_text.strip()}]}
            )

        # Handle interaction if needed
        if self.interaction_config_file:
            add_messages.append({"role": "assistant", "content": response_text})
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        for idx, tool_call in enumerate(agent_data.tool_calls[: self.max_parallel_calls]):
            agent_data.tool_call_counts += 1  # Increment tool call count
        ...
        for tool_response, tool_reward, _ in responses:
            # Handle tool metrics
            if tool_response.metrics:
                if tool_response.name not in agent_data.tool_metrics:
                    agent_data.tool_metrics[tool_response.name] = []
                agent_data.tool_metrics[tool_response.name].append(tool_response.metrics)
        ...
        if self.processor is not None:
            ...
            if self.tool_parser_name == "gpt-oss":
                ...
                for i, tool_msg in enumerate(add_messages):
                    ...
                    formatted = format_gpt_oss_tool_response_manually(
                        tool_msg["content"], actual_tool_name
                    )
        ...
        # Add tool responses to full_messages as user messages
        tool_response_text = await self.loop.run_in_executor(
            None, lambda: self.tokenizer.decode(response_ids, skip_special_tokens=True)
        )
        if tool_response_text.strip():
            tool_user_content = []
            # Check if there are new images in this turn
            if new_images_this_turn:
                tool_user_content.append({"type": "image"})
            tool_user_content.append({"type": "text", "text": tool_response_text.strip()})
            agent_data.full_messages.append({"role": "user", "content": tool_user_content})
    ...
    async def _handle_interacting_state(self, agent_data: AgentData) -> AgentState:
        ...
        if self.processor is not None:
            ...
            model_inputs = self.processor(
                text=[raw_user_response], images=None, return_tensors="pt"
            )
            response_ids = model_inputs.pop("input_ids").squeeze(0).tolist()
        else:
            response_ids = await self.loop.run_in_executor(
    ...
    async def _call_tool(
        ...
        try:
            ...
            if tool_name not in self.tools:
                tool_name = "invalid_func"
            logger.warning(f"Error when executing tool: {e}")
            return (
                ToolResponse(
                    name=tool_name,
                    text=f"Error when executing tool: {e}",
                    metrics={"status": "error", "tool_reward": -1},
                ),
                0.0,
                {"status": "error", "tool_reward": -1},
            )
        finally:
        ...
        if tool_response_text and len(tool_response_text) > self.max_tool_response_length:
            if self.tool_response_truncate_side == "left":
                tool_response_text = (
                    tool_response_text[: self.max_tool_response_length] + "...(truncated)"
                )
            elif self.tool_response_truncate_side == "right":
                tool_response_text = (
                    "(truncated)..." + tool_response_text[-self.max_tool_response_length :]
                )
            else:
        ...
        # current_tool_response_length = 0
        # current_images = []
        # if tool_response_text:
        #     current_tool_response_length += len(tool_response_text)
        # for image in tool_execution_response.image:
        #     if current_tool_response_length > self.max_tool_response_length:
        #         break
        #     image_width, image_length = image.size
        #     current_tool_response_length += (image_width * image_length) / self.config.data.image_patch_size
        # tool_execution_response.image = current_images
        ...
        tool_response_kwargs["metrics"] = tool_metrics
        tool_response_kwargs["name"] = tool_call.name

        return ToolResponse(**tool_response_kwargs), tool_reward, tool_metrics
