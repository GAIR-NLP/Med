...
class RewardModelManager:
    ...
    def _initialize_llm_servers(self):
        ...
        if self.worker_group:
            self._run_all(
                [server.init_colocated(self.worker_group) for server in self.rollout_replicas]
            )
        else:
    ...
    def generate_sequences(self, prompts: DataProto, sampling_params: dict):
        ...
        tasks = [
            self.chat_complete(chat_complete_request)
            for chat_complete_request in chat_complete_requests
        ]
