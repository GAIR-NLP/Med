...
def launch_router_process(
    ...
    router_address = (
        f"[{router_ip}]:{router_port}"
        if is_valid_ipv6_address(router_ip)
        else f"{router_ip}:{router_port}"
    )
...
class NaiveRouter:
    ...
    async def _on_startup(self):
        ...
        if self.verbose:
            logger.info(
                f"[router] aiohttp client initialized with max_connections={self.max_connections}"
            )
