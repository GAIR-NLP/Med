...
class GmmFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, group_list, split_size):
        ...
        outputs = torch_npu.npu_grouped_matmul(
            [x], [weight], group_list=group_list, group_type=0, split_item=2
        )
        return outputs[0]
    ...
    def backward(ctx, grad_outputs):
        ...
        dx = torch_npu.npu_grouped_matmul(
            [grad_outputs], [wt], group_list=group_list, group_type=0, split_item=2
        )
        ...
        with torch.npu.amp.autocast(enabled=False):
            dw = torch.stack(
                [torch.matmul(xt_list[i], grad_outputs_list[i]) for i in range(len(xt_list))]
            )
