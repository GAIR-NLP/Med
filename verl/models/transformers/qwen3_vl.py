    ...
    **kwargs,
    ...
    if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
        ...
        position_ids = torch.ones(
            3, input_ids.shape[0], dtype=input_ids.dtype, device=input_ids.device
        )
...
def _get_input_embeds(
    ...
    if pixel_values_videos is not None:
        ...
        video_embeds, deepstack_video_embeds = model.visual(
    ...
    if pixel_values is None and pixel_values_videos is None:
        ...
        pixel_values = torch.zeros(
            (16, patch_dim), dtype=inputs_embeds.dtype, device=inputs_embeds.device
        )
        image_grid_thw = torch.tensor([[1, 4, 4]], dtype=torch.long, device=inputs_embeds.device)
        image_embeds, dummy_deepstack_image_embeds = model.visual(
