...
from transformers.models.glm46v.modeling_glm46v import (
    Glm46VCausalLMOutputWithPast as Glm4vCausalLMOutputWithPast,
)
from transformers.models.glm46v.modeling_glm46v import (
...
def get_rope_index(
    ...
    if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
        ...
        for modality_type, start_idx, end_idx in input_type_group:
            ...
            if modality_type == "image":
                ...
                for t_idx in range(llm_grid_t):
                    ...
                    h_index = (
                        torch.arange(llm_grid_h).view(1, -1, 1).expand(1, -1, llm_grid_w).flatten()
                    )
                    w_index = (
                        torch.arange(llm_grid_w).view(1, 1, -1).expand(1, llm_grid_h, -1).flatten()
                    )
                    llm_pos_ids_list.append(torch.stack([t_index, h_index, w_index]) + st_idx)
        ...
        if attention_mask is not None:
            ...
            position_ids = (
                torch.arange(input_ids.shape[0], device=input_ids.device).view(1, -1).expand(3, -1)
            )
