...
def model_forward_gen(vision_model: bool = False):
    def model_forward(
        ...
        if "image_grid_thw" in multi_modal_inputs:
            model_kwargs["image_grid_thw"] = multi_modal_inputs["image_grid_thw"].to(
                input_ids.device
            )
        if "pixel_values_videos" in multi_modal_inputs:
            model_kwargs["pixel_values_videos"] = multi_modal_inputs["pixel_values_videos"].to(
                input_ids.device
            )
        if "video_grid_thw" in multi_modal_inputs:
            model_kwargs["video_grid_thw"] = multi_modal_inputs["video_grid_thw"].to(
                input_ids.device
            )
...
def gptmodel_forward_no_padding(
    ...
    input_ids_rmpad, packed_seq_params = preprocess_packed_seqs_no_padding(
    ...
    if post_process and logits_processor is not None:
        args = {
            k: preprocess_packed_seqs_no_padding(v, pre_process=True)[0]
            for k, v in logits_processor_args.items()
        }
        output_dict = logits_processor(output_orig, **args)
        output = {
