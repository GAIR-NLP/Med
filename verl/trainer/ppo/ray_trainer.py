...
def filter_overlong_samples(batch, max_turns=None):
    """Filter out samples that reach maximum length or maximum turns by setting their response_mask to all zeros.

    If a sample's response_mask is all 1s, it indicates the sample reached maximum length,
    and we should mask it out (set to all 0s) to avoid computing loss on it.

    Additionally, if the number of turns exceeds the maximum allowed turns, the sample is also masked out.

    Args:
        batch (DataProto): The batch containing response_mask and turn information.
        max_turns (int, optional): Maximum allowed number of turns.
    """
    response_mask = batch.batch["response_mask"]

    # Check which samples have all 1s (reached max length)
    all_ones_mask = response_mask.sum(dim=-1) == response_mask.size(-1)

    # Create a copy to avoid in-place modification
    masked_response_mask = response_mask.clone()

    # Set response_mask to all 0s for samples that reached max length
    masked_response_mask[all_ones_mask] = 0

    # Check for maximum turns if specified
    if max_turns is not None and "__num_turns__" in batch.non_tensor_batch:
        num_turns = batch.non_tensor_batch["__num_turns__"]

        # Find samples that exceed maximum turns
        for i, turns in enumerate(num_turns):
            if turns >= max_turns:
                masked_response_mask[i] = 0

    # Update the batch with filtered response_mask
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            sample_result_dicts.extend(result["reward_extra_info"]["result_dicts"])
        ...
        for result in sample_result_dicts:
            ...
            for key, value in result.items():
                if "reward" in key and value is not None:
                    data_source_reward_dict[data_source].setdefault(key, []).append(value)
        ...
        for data_source, source_rewards in data_source_reward_dict.items():
            for reward_name, rewards in source_rewards.items():
                ...
                reward_name = reward_name.replace("/", "_")
                key = f"{data_split}_{reward_name}/{data_source}"
                metric_dict[key] = np.mean(rewards)

        # verl写的真的太烂了，无力吐槽
        # data_sources = np.concatenate(data_source_lst, axis=0)
        # infos_dict = defaultdict(list)
        # for result_dict in reward_extra_infos_dict["result_dicts"]:
        #     infos_dict["score"].append(result_dict["final_reward"])
    ...
    def _balance_batch(
        self, batch: DataProto, metrics, logging_prefix="global_seqlen", keep_minibatch=False
    ):
    ...
    def fit(self):
        ...
        logger = Tracking(
            project_name=self.config.trainer.project_name,
            experiment_name=f"{self.config.trainer.experiment_name}_{self.timestamp}",
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Filter out samples that reach maximum length
                    if self.config.trainer.filter_overlong_mask:
                        max_turns = (
                            self.config.actor_rollout_ref.rollout.multi_turn.max_user_turns
                            + self.config.actor_rollout_ref.rollout.multi_turn.max_assistant_turns
                        )
                        filter_overlong_samples(batch, max_turns)
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            result_dicts = reward_extra_infos_dict.pop("result_dicts")
