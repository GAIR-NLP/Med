...
def compute_rollout_rejection_mask(
    ...
    lower_threshold = (
        ...
        if rollout_rs_threshold_lower is not None
        else 1.0 / upper_threshold
    )
    ...
    if rollout_rs == "token":
        ...
        log_ratio_sum: torch.Tensor = verl_F.masked_sum(
            log_ratio, response_mask, axis=-1
        ).unsqueeze(-1)  # Shape: (batch_size, 1)
        log_ratio_for_metrics = log_ratio_sum

        log_ratio_sum_safe: torch.Tensor = torch.clamp(
            log_ratio_sum, min=-SAFETY_BOUND, max=SAFETY_BOUND
        )
        rollout_is_weights = torch.exp(log_ratio_sum_safe).expand_as(
            log_ratio
        )  # Broadcast to (batch_size, seq_length)

    elif rollout_rs == "geometric":
        # Sequence-level geometric mean: exp(mean(log ratios))
        log_ratio_mean: torch.Tensor = verl_F.masked_mean(
            log_ratio, response_mask, axis=-1
        ).unsqueeze(-1)  # Shape: (batch_size, 1)
        log_ratio_for_metrics = log_ratio_mean

        log_ratio_mean_safe: torch.Tensor = torch.clamp(
    ...
    mask: torch.Tensor = (rollout_is_weights >= lower_threshold) & (
...
def compute_is_metrics(
    ...
    log_threshold_lower: torch.Tensor = torch.log(
        torch.tensor(rollout_is_threshold_lower, device=device)
    )
    ...
    if rollout_is == "sequence":
        ...
        mask_bool: torch.Tensor = response_mask.bool()
        metrics["rollout_is_max"] = (
            rollout_is_weights.masked_fill(~mask_bool, float("-inf")).max().item()
        )
        metrics["rollout_is_min"] = (
            rollout_is_weights.masked_fill(~mask_bool, float("inf")).min().item()
        )
    ...
    if rollout_is_weights.dim() > 1:
        ...
        # Fraction of sequences with extreme weights
        metrics["rollout_is_seq_fraction_high"] = (
            (seq_mean_weights > rollout_is_threshold).float().mean().item()
        )
        metrics["rollout_is_seq_fraction_low"] = (
            (seq_mean_weights < rollout_is_threshold_lower).float().mean().item()
        )
...
def compute_rollout_correction_and_rejection_mask(
    ...
    if rollout_token_veto_threshold is not None:
        ...
        log_veto_threshold: torch.Tensor = torch.log(
            torch.tensor(rollout_token_veto_threshold, device=device)
        )
    ...
    if rollout_is_weights is not None:
        rollout_is_weights_proto = DataProto.from_dict(
...
def compute_offpolicy_metrics(
    ...
    mean_log_prob_training = verl_F.masked_mean(
        old_log_prob, response_mask, axis=-1
    )  # (batch_size,)
    ...
    if rollout_log_prob is not None:
        ...
        metrics["kl"] = (
            verl_F.masked_mean(rollout_log_prob - old_log_prob, response_mask).detach().item()
        )
        ...
        mean_log_prob_rollout = verl_F.masked_mean(
            rollout_log_prob, response_mask, axis=-1
        )  # (batch_size,)
