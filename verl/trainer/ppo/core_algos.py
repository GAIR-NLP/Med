...
def get_kl_controller(kl_ctrl):
    ...
    if kl_ctrl.type == "fixed":
        ...
        return AdaptiveKLController(
            init_kl_coef=kl_ctrl.kl_coef, target_kl=kl_ctrl.target_kl, horizon=kl_ctrl.horizon
        )
    else:
...
def compute_policy_loss_gspo(
    ...
    clip_ratio_low = (
        config.clip_ratio_low if config.clip_ratio_low is not None else config.clip_ratio
    )
    clip_ratio_high = (
        config.clip_ratio_high if config.clip_ratio_high is not None else config.clip_ratio
    )
...
def compute_policy_loss_clip_cov(
    ...
    clip_cov_ub = (
        config.policy_loss.clip_cov_ub if config.policy_loss.clip_cov_ub is not None else 5.0
    )
    clip_cov_lb = (
        config.policy_loss.clip_cov_lb if config.policy_loss.clip_cov_lb is not None else 1.0
    )
...
def compute_policy_loss_kl_cov(
    ...
    kl_cov_ratio = (
        config.policy_loss.kl_cov_ratio if config.policy_loss.kl_cov_ratio is not None else 0.0002
    )
    ppo_kl_coef = (
...
def compute_policy_loss_geo_mean(
    ...
    if rollout_is_weights is not None:
        ...
        seq_is_weights = torch.exp(
            (torch.log(rollout_is_weights + 1e-10) * response_mask).sum(dim=-1)
...
def compute_policy_loss_rollout_correction_wrapper(
    ...
    rollout_corr_config = (
        config.policy_loss.get("rollout_correction", None)
