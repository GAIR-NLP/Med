...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        with init_context():
            ...
            if self.use_remove_padding or self.config.ulysses_sequence_parallel_size > 1:
                ...
                apply_monkey_patch(
                    model=self.model, ulysses_sp_size=self.config.ulysses_sequence_parallel_size
                )
            ...
            if self.lora:
                ...
                if lora_adapter_path is not None:
                    ...
                    self.model = PeftModel.from_pretrained(
                        self.model, local_adapter_path, is_trainable=True
                    )
    ...
    def save_checkpoint(self, step):
        ...
        if self.device_mesh.get_rank() == 0 and getattr(
            self.config.trainer, "default_hdfs_dir", None
        ):
            hdfs_io.makedirs(self.config.trainer.default_hdfs_dir, exist_ok=True)
            hdfs_io.copy(
                src=local_global_step_folder,
    ...
    def _determine_resume_path(self):
        ...
        if resume_mode == "disable":
            ...
            if resume_from_path is not None:
                ...
                assert (
                    "global_step_" in resume_from_path
                ), "resume_from_path must specify the global_steps"
            ...
            raise ValueError(
                f"Invalid resume_mode: {resume_mode}. Must be 'auto', 'disable', or 'resume_path'"
            )
...
def create_sft_dataset(data_paths, data_config, tokenizer, max_samples=-1):
    ...
    dataset = dataset_cls(
        parquet_files=data_paths, tokenizer=tokenizer, config=data_config, max_samples=max_samples
    )
    return dataset
