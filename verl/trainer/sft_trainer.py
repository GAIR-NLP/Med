...
class SFTTrainer:
    ...
    def fit(self):
        is_logging = (
            self.engine.is_mp_src_rank_with_outputs() and self.engine.get_data_parallel_rank() == 0
        )
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                if self.engine.is_mp_src_rank_with_outputs():
                    ...
                    metrics["train/mfu"] = (
                        estimated_flops / promised_flops / torch.distributed.get_world_size()
                    )
                    ...
                    for val_data in self.val_dataloader:
                        with self.engine.eval_mode():
                            ...
                            output = self.engine.infer_batch(
                                data=val_data, loss_function=self.loss_fn
                            )
                            if self.engine.is_mp_src_rank_with_outputs():
