...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_engine(self):
        ...
        if "ForCausalLM" in arch:
            ...
            warnings.warn(
                f"Implicitly changing critic architecture from '{arch}' to '{new_arch}'",
                stacklevel=2,
            )
        ...
        assert (
            self.ppo_mini_batch_size % self.engine.get_data_parallel_size() == 0
        ), f"{self.ppo_mini_batch_size=} is not divisible by {self.engine.get_data_parallel_size()=}"
        self.ppo_mini_batch_size_per_dp = (
            self.ppo_mini_batch_size // self.engine.get_data_parallel_size()
        )
    ...
    def compute_values(self, data: DataProto):
        ...
        if self.config.use_dynamic_bsz:
            ...
            ] = self.config.ppo_infer_micro_batch_size_per_gpu
