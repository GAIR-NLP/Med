...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            #     if optim_config.lr_freeze is not None and len(optim_config.lr_freeze) > 0:
            #         if len(optim_config.lr_freeze) == 3:
            #             raise NotImplementedError(
            #                 f'All part are frozen, lr_freeze={optim_config.lr_freeze} is not supported')
            #
            #         freeze_param_names = {  # Store names for logging
            #             'vit': [],
            #             'connector': [],
            #             'llm': [],
            #         }
            #         if self.rank == 0:
            #             logger.warning(
            #                 f"Attempting to freeze parameters based on optim_config.lr_freeze={optim_config.lr_freeze}")
            #
            #         for name, param in actor_module.named_parameters():
            #             try:
            #                 model_name, module_name = name.split('.')[1:3]
            #             except:
            #                 model_name, module_name = name.split('.')[0:2]
            #
            #             # Check visual parts first
            #             if model_name == 'visual':
            #                 if (module_name == 'blocks' or module_name == 'patch_embed'):
            #                     if 'vit' in optim_config.lr_freeze:
            #                         param.requires_grad = False
            #                         freeze_param_names['vit'].append(name)
            #                 elif module_name == 'merger':
            #                     if 'connector' in optim_config.lr_freeze:
            #                         param.requires_grad = False
            #                         freeze_param_names['connector'].append(name)
            #                 else:
            #                     raise NotImplementedError(
            #                         f'Model name {model_name} and module name {module_name} is not supported')
            #             elif model_name == 'language_model':
            #                 if 'llm' in optim_config.lr_freeze:
            #                     param.requires_grad = False
            #                     freeze_param_names['llm'].append(name)
            #             elif model_name == 'lm_head':
            #                 if 'llm' in optim_config.lr_freeze:
            #                     param.requires_grad = False
            #                     freeze_param_names['llm'].append(name)
            #             else:
            ...
            #
            #         if self.rank == 0:
            #             logger.warning('Vit frozen: %s', freeze_param_names['vit'])
            #             logger.warning('Connector frozen: %s', freeze_param_names['connector'])
            #             logger.warning('Llm frozen: %s', freeze_param_names['llm'])
            #     else:
            #         if self.rank == 0:
            #             logger.warning('No parameters frozen')
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        infer_tp = (
            self.config.rollout.tensor_model_parallel_size * self.config.rollout.data_parallel_size
        )
    ...
    async def rollout_mode(self):
        ...
        if peft_config is not None and getattr(self.rollout, "sleep_level", None) == 2:
            ...
            base_model_params = {
                replace_lora_wrapper(k, peft_config): v for k, v in base_model_params.items()
            }
