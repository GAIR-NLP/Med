    ...
    def _build_fsdp_module(self, module):
        ...
        if self.model_config.enable_activation_offload:
            ...
            enable_activation_offloading(
                module, self.engine_config.strategy, enable_gradient_checkpointing
            )
...
class EngineTrainModeCtx:
    ...
    def __enter__(self):
        ...
        if self.engine._is_offload_optimizer:
            load_fsdp_optimizer(
                optimizer=self.engine.optimizer, device_id=get_torch_device().current_device()
            )
...
@EngineRegistry.register(
    model_type="language_model", backend=["fsdp", "fsdp2"], device=["cuda", "npu"]
)
class FSDPEngineWithLMHead(FSDPEngine):
    def prepare_model_inputs(self, micro_batch: TensorDict):
        ...
        if use_remove_padding:
            ...
            if pad_mode == DatasetPadMode.NO_PADDING:
                ...
                pad_token_id = tu.get_non_tensor_data(
                    data=micro_batch, key="pad_token_id", default=0
                )
                ...
                attention_mask = torch.nested.as_nested_tensor(
                    attention_mask_list, layout=torch.jagged
                )
    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict):
        ...
        if use_remove_padding:
            ...
            response_length = tu.get_non_tensor_data(
                data=micro_batch, key="max_response_length", default=1024
            )
            if use_fused_kernels:
                ...
                if pad_mode == DatasetPadMode.NO_PADDING:
                    ...
                    log_probs = torch.nested.nested_tensor_from_jagged(log_probs, cu_seqlens)
                    if calculate_entropy:
                        entropy = torch.nested.narrow(
                            entropy, 1, starts, seq_lengths, layout=torch.jagged
                        )
...
@EngineRegistry.register(
    model_type="value_model", backend=["fsdp", "fsdp2"], device=["cuda", "npu"]
)
