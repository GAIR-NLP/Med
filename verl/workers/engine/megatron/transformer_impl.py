...
class MegatronEngine(BaseEngine):
    ...
    def _build_tf_config(self):
        ...
        if not self.bridge:
            self.weight_converter = get_mcore_weight_converter(
                self.model_config.hf_config, self.dtype
            )
        ...
        if self.engine_config.use_dist_checkpointing:
            ...
            if self.bridge is not None:
                ...
                tmp_config = OmegaConf.create(
                    {
                        "model": {
                            "path": self.model_config.local_path,
    ...
    def _build_optimizer(self):
        ...
        optim_config_megatron = init_megatron_optim_config(
            self.optimizer_config, self.param_dtype == torch.float16
        )
    ...
    def postprocess_micro_batch_func(
        self, output, data: TensorDict, forward_only: bool, loss_function
    ):
