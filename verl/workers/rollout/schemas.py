...
class AsyncRolloutRequest(BaseModel):
    ...
    def initialize_request(cls, values):
        ...
        tools = (
            [tool.model_dump(exclude_none=True) for tool in tool_schemas]
    ...
    def _handle_apply_chat_template(
        ...
        trim_last_line_break: bool = False,
    ):
        ...
        if trim_last_line_break and raw_prompt[-11:] == "<|im_end|>\n":
    ...
    def get_generation_prompt_ids(
        ...
        if generation_prompt_ids is not None:
            self._update_input_ids(
                processing_class, generation_prompt_ids, attention_mask=True, loss_mask=False
            )

        if self.use_inference_chat_template:
            messages = [msg.model_dump(exclude_none=True) for msg in self.messages]
            tools = (
                [tool.model_dump(exclude_none=True) for tool in self.tool_schemas]
    ...
    def add_assistant_message(
        ...
        # only execute the first function call
    ...
    def add_tool_response_messages(
        ...
        tool_response_len = content_ids.shape[-1]

        response_len = current_response_len + tool_response_len
        if response_len > self.max_response_len:
            logger.warning(
                f"The visual tool response exceeded the maximum response length,"
                f"therefore the processed image will be discarded: {response_len} > {self.max_response_len}"
            )
            before_tool_response_messages = self.messages[: -len(contents)]
            raw_query = ""
            for content in self.messages[0].content:
                if content["type"] == "text":
                    raw_query += content["text"]
            new_content = f"Due to the context length limit, the image returned by the crop_and_zoom tool cannot be obtained.\n\nPlease continue to answer the original question: {raw_query}"
            self.messages = before_tool_response_messages + [
                Message(role="tool", content=new_content)
            ]
            messages = [*BASE_CHAT_HISTORY, *self.messages[-1:]]
            delta_multi_modal_data = {key: [] for key in self.multi_modal_keys}
            ...
            if diffs := self._get_prompt_diffs(
                ...
                if self.tokenization_sanity_check_mode == TokenizationSanityCheckModeEnum.STRICT:
                    ...
                    non_strippable_diffs_exist = any(
                        d["full_prompt_chunk"].strip() or d["current_prompt_chunk"].strip()
                        for d in diffs
                    )
    ...
    def truncate_output_ids(
        ...
        self.response_loss_mask = self.loss_mask[..., self.prompt_loss_mask.shape[-1] :][
            ..., : self.max_response_len
        ]
