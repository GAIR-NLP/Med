...
def _glm46v_dedup_image_tokens(prompt_ids: list[int], processor):
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        if "enable_weights_cpu_backup" in [f.name for f in dataclasses.fields(ServerArgs)]:
            enable_weights_cpu_backup = (
                True if self.rollout_mode == RolloutMode.COLOCATED else False
            )
    ...
    async def launch_servers(self):
        ...
        for node_rank in range(self.nnodes):
            workers = self.workers[
                node_rank * self.gpus_per_node : (node_rank + 1) * self.gpus_per_node
            ]
