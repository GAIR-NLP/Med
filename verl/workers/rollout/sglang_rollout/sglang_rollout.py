...
GLOBAL_PROFILER_STATS = {
    "total_calls": 0,
    "total_duration": 0.0,
    "time_spent_in_engine": 0.0,
    "time_spent_in_tools": 0.0,
    "time_spent_in_interaction": 0.0,
    "time_spent_in_reward_calc": 0.0,
    "cpu_logic_duration": 0.0,
}
...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        # Initialize token IDs for token replacement in outputs
        self.image_token_id = (
            model_hf_config.image_token_id if hasattr(model_hf_config, "image_token_id") else None
        )
        self.video_token_id = (
            model_hf_config.video_token_id if hasattr(model_hf_config, "video_token_id") else None
        )

        # Get tokenizer for encoding a space token as replacement
        try:
            # This is when processing_class is a tokenizer
            self.replace_token_id = self.processing_class.encode(" ")[0]
        except AttributeError:
            try:
                # This is when processing_class is a processor
                self.replace_token_id = self.processing_class.tokenizer.encode(" ")[0]
            except AttributeError:
                # Fallback: use a common space token ID
                self.replace_token_id = 220  # Common space token ID for many models
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    tool_call_results = await asyncio.gather(
                        *[
                            self._tool_map[tool_call.function.name].execute(
                                ...
                                **_req.tools_kwargs.get(tool_call.function.name, {}).get(
                                    "execute_kwargs", {}
                                ),
                            )
                    ...
                    for tool_call, (_, _, metrics) in zip(
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    _req.add_assistant_message(
                        self.processing_class, content=content, content_ids=content_ids
                    )
                    break
                else:
    ...
    async def _handle_engine_generate(
        ...
        max_new_tokens = min(
            self.config.response_length, self.config.max_model_len - len(generation_prompt_ids) - 1
        )
        ...
        if "text" in output:
            output["text"] = output["text"].replace("<image>", "")
            output["text"] = output["text"].replace("<video>", "")
            output["text"] = output["text"].replace("<|image_pad|>", "")
            output["text"] = output["text"].replace("<|video_pad|>", "")
        return output
    ...
    async def _handle_pending_state(self, _req: AsyncRolloutRequest) -> AsyncRolloutRequest:
        if _req.tool_schemas is not None:
            ...
            for tool_schema in _req.tool_schemas:
                ...
                tool_creation_coroutines.append(tool.create(create_kwargs, _req.request_id))
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._tp_rank == 0:
            ...
            if is_validate:
                ...
                async def rollout_a_request_with_cancellation_handler(req):
                    try:
                        result = await self._async_rollout_a_request(
                ...
                async def run_with_cancellation():
                    ...
                    all_tasks = [
                        asyncio.create_task(rollout_a_request_with_cancellation_handler(req))
                        for req in req_list
                    ]
        ...
        for req in sorted_output_req_list:
            ...
            position_ids_seq_len = (
                len(req.position_ids[0])
                if isinstance(req.position_ids[0], list)
                else len(req.position_ids)
            )
            assert (
            ...
            multi_modal_data.append(req.multi_modal_data)
            if self.config.calculate_log_probs:
        ...
        if is_multimodal:
            ...
            non_tensor_batch["multi_modal_data"] = np.array(multi_modal_data, dtype=object)
