...
def launch_server_process(
    ...
    if server_args.node_rank != 0 or not first_rank_in_node:
        logger.info(
            f"Server process started with PID {p.pid} for node rank {server_args.node_rank}",
            flush=True,
        )
        return p
            ...
            try:
                if server_args.is_embedding:
                    ...
                    response = session.get(
                        f"{base_url}/health_generate", headers=headers, timeout=timeout
                    )
...
class HttpServerAdapter(EngineBase):
    ...
    def _make_request(
        ...
        raise RuntimeError(
            f"Failed to complete request to {endpoint} after {self.max_attempts} attempts"
        )
...
class AsyncHttpServerAdapter(HttpServerAdapter):
                ...
                async with self._get_session() as session:
                    if method.upper() == "GET":
                        ...
                        async with session.post(
                            url, json=payload or {}, timeout=timeout
                        ) as response:
                            response.raise_for_status()
                            return await _read_async_response(response)
    ...
    async def reward_score(
        ...
        # Send request
        response = await self._make_async_request(
            "classify", payload, timeout=self.timeout, only_master=False
        )
