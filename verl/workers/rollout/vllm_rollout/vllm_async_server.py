...
class vLLMHttpServerBase:
    ...
    def __init__(
        ...
        if self.rollout_mode != RolloutMode.HYBRID and self.config.load_format == "dummy":
            logger.warning(
                f"rollout mode is {self.rollout_mode}, load_format is dummy, set to auto"
            )
        ...
        self.image_token_id = (
            self.model_config.hf_config.image_token_id
            if "image_token_id" in self.model_config.hf_config
            else None
        )
        self.video_token_id = (
            self.model_config.hf_config.video_token_id
            if "video_token_id" in self.model_config.hf_config
            else None
        )
        # FIXME: 220 is the space token of Qwen, but has to fix it when other model.
        self.replace_token_id = 220
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        if self.node_rank != 0:
            assert (
                master_address and master_port
            ), "non-master node should provide master address and port"
        ...
        if self.config.get("limit_images", None):  # support for multi-image data
            engine_kwargs["limit_mm_per_prompt"] = json.dumps(
                {"image": self.config.get("limit_images")}
            )
        ...
        if self.config.expert_parallel_size > 1:
            ...
            assert (
                len(self.workers)
                == data_parallel_size_local * self.config.tensor_model_parallel_size
        ...
        distributed_executor_backend = (
            ExternalZeroMQDistributedExecutor if len(self.workers) > 0 else None
        )
    ...
    async def generate(
        ...
        # Replace image and video tokens in response if they appear
        if self.image_token_id is not None and self.image_token_id in token_ids:
            # Replace all occurrences of image_token_id with replace_token_id using numpy
            token_ids = np.array(token_ids)
            count = np.sum(token_ids == self.image_token_id)
            token_ids[token_ids == self.image_token_id] = self.replace_token_id
            token_ids = token_ids.tolist()
            logger.warning(
                f"Appeared image token id in response: {self.image_token_id}, count: {count}, replace it with empty token id {self.replace_token_id}"
            )
        if self.video_token_id is not None and self.video_token_id in token_ids:
            token_ids = np.array(token_ids)
            count = np.sum(token_ids == self.video_token_id)
            token_ids[token_ids == self.video_token_id] = self.replace_token_id
            token_ids = token_ids.tolist()
            logger.warning(
                f"Appeared video token id in response: {self.video_token_id}, count: {count}, replace it with empty token id {self.replace_token_id}"
            )
...
def _qwen2_5_vl_dedup_image_tokens(prompt_ids: list[int], processor):
    ...
    elif processor is not None and (
