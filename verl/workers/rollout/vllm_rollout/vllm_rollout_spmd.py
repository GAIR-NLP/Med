...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        self.image_token_id = (
            model_hf_config.image_token_id if "image_token_id" in model_hf_config else None
        )
        self.video_token_id = (
            model_hf_config.video_token_id if "video_token_id" in model_hf_config else None
        )
        self.replace_token_id = tokenizer.encode(" ")[0]
        ...
        with self.update_sampling_params(**kwargs):
            ...
            for output in outputs:
                for sample in output.outputs:
                    response_token_ids: List[int] = sample.token_ids
                    if self.config.calculate_log_probs:
                        curr_log_prob = []
                        for i, logprob in enumerate(sample.logprobs):
                            curr_log_prob.append(logprob[response_token_ids[i]].logprob)
                        rollout_log_probs.append(curr_log_prob)

                    if (
                        self.image_token_id is not None
                        and self.image_token_id in response_token_ids
                    ):
                        # Replace all occurrences of image_token_id with replace_token_id using numpy
                        response_token_ids = np.array(response_token_ids)
                        count = np.sum(response_token_ids == self.image_token_id)
                        response_token_ids[
                            response_token_ids == self.image_token_id
                    ...
                    if (
                        self.video_token_id is not None
                        and self.video_token_id in response_token_ids
                    ):
                        response_token_ids = np.array(response_token_ids)
                        count = np.sum(response_token_ids == self.video_token_id)
                        response_token_ids[
                            response_token_ids == self.video_token_id
                        ] = self.replace_token_id
                        response_token_ids = response_token_ids.tolist()
                        logger.warning(
    ...
    async def update_weights(
        ...
        peft_config, base_sync_done = (
            kwargs.get("peft_config", None),
            kwargs.get("base_sync_done", False),
        )
...
class vLLMAsyncRollout(BaseRollout):
    ...
    def __init__(
        ...
        if config.layered_summon or (
            config.expert_parallel_size > 1 and not _check_vllm_version_for_sleep_level()
        ):
