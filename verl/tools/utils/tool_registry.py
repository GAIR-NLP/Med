...
async def initialize_mcp_tool(tool_cls, tool_config) -> list:
    ...
    tool_selected_list = (
        tool_config.mcp.tool_selected_list if "tool_selected_list" in tool_config.mcp else None
    )
...
def initialize_tools_from_config(tools_config_file):
    ...
    thread = threading.Thread(
        target=tmp_event_loop.run_forever, name="mcp tool list fetcher", daemon=True
    )
