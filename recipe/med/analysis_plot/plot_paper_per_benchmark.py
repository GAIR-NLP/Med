...
def plot_paper_per_benchmark(
    ...
    fig, axes = plt.subplots(2, 3, figsize=(18, 12), dpi=PAPER_FIG_CONFIG["dpi"], facecolor="white")
    ...
    for bench_idx, benchmark in enumerate(target_benchmarks):
        ...
        agg_data = aggregate_benchmarks(df, [benchmark], smoothing_factor, smoothing_method)
        ...
        if "w_tool_score" in benchmark_data.columns and "wo_tool_score" in benchmark_data.columns:
            ...
            if smoothing_factor > 0 or smoothing_method != "none":
                w_tool_abs = smooth_curve(w_tool_abs_raw, steps, smoothing_factor, smoothing_method)
        ...
        ax.set_ylabel("Accuracy", fontsize=PAPER_FIG_CONFIG["label_fontsize"], fontweight="medium")
        ax.grid(
...
def main():
    parser = argparse.ArgumentParser(description="Plot per-benchmark absolute performance")
