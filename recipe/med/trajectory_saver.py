...
def save_trajectories_jsonl(
    ...
    with open(jsonl_file, "w", encoding="utf-8") as f:
        for i, traj in enumerate(trajectories_list):
            ...
            tool_call_counts = samples_list[i].non_tensor_batch.get(
                "tool_call_counts", 0
            )
...
def extract_text_data(sample, tokenizer, dump_path, exp_name, step, save_images=False):
    ...
    response_mask = torch.cat(
        [torch.zeros(prompt_length), sample.batch["response_mask"]]
    )
...
def convert_batch_to_trajectory(
    ...
    print(
        f"Time to calculate all response lengths: {time.time() - time_len_calc_start:.4f}s"
    )
    ...
    if no_tool_call_samples:
        ...
        print(
            f"Time for 'No Tool Call' processing: {time.time() - time_no_tool_call_start:.4f}s"
        )
        print("-" * 30)
    ...
    for category, samples in sample_categories.items():
        if samples:  # 只为非空的类别生成轨迹
            ...
            print(
                f"Generated {len(trajectories[category])} trajectories for {category}"
            )
        else:
            trajectories[category] = []
    ...
    for category, category_trajectories in trajectories.items():
        if category_trajectories:  # 只保存非空的轨迹
            ...
            sample_result_dicts = [
                result_dicts[idx] for idx, _ in sample_categories[category]
            ]
            ...
            print(
                f"Saved {category}_trajectories.jsonl with {len(category_trajectories)} samples"
            )
        else:
