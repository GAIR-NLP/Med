...
# --- Script Body ---
echo "🚀 Starting the dynamic Ray Serve deployment script..."

# 1. Check Ray cluster connection
echo "Checking Ray cluster connection..."
if ! ray health-check > /dev/null; then
    echo "❌ Error: Could not connect to the Ray cluster."
    exit 1
fi
...
if [ -z "$TOTAL_CPUS_FLOAT" ]; then
    echo "❌ Error: Failed to parse total CPUs from 'ray status' command."
    exit 1
fi
TOTAL_CPUS=$(printf "%.0f" "$TOTAL_CPUS_FLOAT")
echo "✅ Detected a total of ${TOTAL_CPUS} CPU cores in the cluster."

# RESERVED_CPUS=36

# 3. Calculate the number of replicas
echo "🧮 Calculating the number of application replicas (num_replicas)..."
...
echo "✅ Final number of replicas to deploy: ${NUM_REPLICAS}."

# 4. Generate the deployment YAML using `env_vars`
echo "📄 Generating deployment configuration file: ${CONFIG_FILE}..."
cat > "$CONFIG_FILE" << EOF
...
echo "✅ Configuration file generated successfully using env_vars."
...
echo "🧹 Cleaning up temporary configuration file..."
if [ -f "${CONFIG_FILE}" ]; then
    rm "${CONFIG_FILE}"
    echo "✅ Cleanup complete."
else
    echo "⚠️ Configuration file not found, skipping cleanup."
fi
